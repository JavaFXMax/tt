...
class VehicleIncomesController extends \BaseController {
	...
	public function store()
	{
        ...
        if(Input::get('petrol_investment') > 0 ){
            $data = array(
                ...
                'debit_account' =>Input::get('asset_id'),
                'credit_account' => Input::get('eq_id'),
	...
	public function destroy($id)
	{
        $vehicle = Vehicleincome::findOrFail($id);
				...
				if($counter >0){
					    $saving = Savingtransaction::where('id',$vehicle->savingtransaction_id)->get()->first();
			        $saving->void = 1;
							$saving->update();
				}
		...
		if($vehicle->loantransaction_id != null || $vehicle->loantransaction_id != '' || $vehicle->loantransaction_id != 0){
				...
				if($saving_count >0){
							$saving = Loantransaction::where('id',$vehicle->loantransaction_id)->get()->first();
			        $saving->void = 1;
							$saving->update();
				}
				...
				if($loan_count >0){
							$saving = Loanrepayment::where('id',$vehicle->loanrepayment_principal_id)->get()->first();
			        $saving->void = 1;
							$saving->update();
				}
				...
				if($repay_count>0){
							$saving = Loanrepayment::where('id',$vehicle->loanrepayment_interest_id)->get()->first();
					    $saving->void = 1;
							$saving->update();
				}
				...
				if($journal_count>0){
							$journal = Journal::where('id',$vehicle->interest_credit_journal_id)->get()->first();
			        $journal->void = 1;
			        $journal->update();
				}
				...
				if($journal_count_1 > 0){
			        $journal = Journal::where('id',$vehicle->interest_debit_journal_id)->get()->first();
			        $journal->void = 1;
			        $journal->update();
				}
				...
				if($journal_count_2 > 0){
			        $journal = Journal::where('id',$vehicle->principal_credit_journal_id)->get()->first();
			        $journal->void = 1;
			        $journal->update();
				}
				...
				if($journal_count_3 > 0){
			        $journal = Journal::where('id',$vehicle->principal_debit_journal_id)->get()->first();
			        $journal->void = 1;
			        $journal->update();
				}
				...
				if($journal_savings_count > 0){
					  	$journal = Journal::where('id',$vehicle->savings_credit_journal_id)->get()->first();
			        $journal->void = 1;
			        $journal->update();
				}
				...
				if($journal_savings_debit_count > 0){
							$journal = Journal::where('id',$vehicle->savings_debit_journal_id)->get()->first();
							$journal->void = 1;
							$journal->update();
				}
				...
				if($journal_asset_count > 0){
							$journal = Journal::where('id',$vehicle->asset_journal_id)->get()->first();
							$journal->void = 1;
							$journal->update();
				}
				...
				if($journal_income_count > 0){
							$journal = Journal::where('id',$vehicle->income_journal_id)->get()->first();
							$journal->void = 1;
							$journal->update();
				}
				...
				if($journal_equity_credit_count > 0){
							$journal = Journal::where('id',$vehicle->equity_credit_journal_id)->get()->first();
							$journal->void = 1;
							$journal->update();
				}
				...
				if($journal_equity_debit_count > 0){
							$journal = Journal::where('id',$vehicle->equity_debit_journal_id)->get()->first();
							$journal->void = 1;
							$journal->update();
				}
