...
/*
   ...
   // {
	...
	public static function getPrincipalBal($loanaccount){
						$transaction = Loantransaction::where('description','=','loan arrears')
						->where('loanaccount_id',$loanaccount->id)->count();
						if($transaction > 0){
									$transaction_amount = Loantransaction::where('description','=','loan arrears')
									->where('loanaccount_id',$loanaccount->id)->sum('amount');
									...
									$principal_bal = $principal_balance + $transaction_amount;
