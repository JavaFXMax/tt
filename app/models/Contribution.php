...
class Contribution extends \Eloquent {
	...
	public static function credittotal($id,$from,$to){
		$total = DB::table('contributions')->where('member_id', '=', $id)->whereBetween('date', array($from, $to))->where('type', '=', 'credit')
		->where('is_void',false)->sum('amount');
		return $total;
	}

	public static function sum($id){
					$credit_total= DB::table('contributions')->where('member_id', '=', $id)->where('type', '=', 'credit')
					->where('is_void',0)->sum('amount');
					$debit_total = DB::table('contributions')->where('member_id', '=', $id)->where('type', '=', 'debit')
					->where('is_void',0)->sum('amount');
