...
class Account extends \Eloquent {
	...
	public static function getAccountBalanceAtDate($account, $date){
		...
		$balance = 0;
		$credit = DB::table('journals')->where('void', '=', 0)->where('account_id', '=', $account->id)->where('type', '=', 'credit')->where('date', '<=', $date)->sum('amount');
		$debit = DB::table('journals')->where('void', '=', 0)->where('account_id', '=', $account->id)->where('type', '=', 'debit')->where('date', '<=', $date)->sum('amount');
